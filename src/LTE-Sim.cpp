	  ...
	  if (strcmp(argv[1], "MultiCell")==0)
	    {
		  ...
		  int bandwidth = atoi(argv[19]);
	      int seed = -1;
	      if (argc==21) seed = atoi(argv[20]);
	      MultiCell (nbCells, radius, nbUE, nbVoIP, nbVideo, nbBE, nbCBR, sched_type, frame_struct, speed, maxDelay, video_bit_rate, config_fname, num_macro, num_micro, inter_micro_distance, duration, bandwidth, seed);
	    }
