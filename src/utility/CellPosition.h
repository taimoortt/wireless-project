...
std::vector<CartesianCoordinates*>* GetCartesianCoordinatesForMicro(CartesianCoordinates* macro, int n, int radius, int inter_micro_dist) {
  ...
  bool isFarEnough;
   for (int i = 0; i < n; i++){
    ...
    std::cout << "New CDS from Macro: " << macro_distance << "\n";
    if(macro_distance < 75.0 || macro_distance > 1000.0){
      std::cout << "Not far enough from a Macro\n";
      isFarEnough = false;
      i--;
    } else {
      for (int j = 0; j < vectorOfCoordinates->size(); j++){
        ...
        if (micro_distance < inter_micro_dist){
          std::cout << "Not far enough from a Micro\n";
          isFarEnough = false;
          i--;
          break;
        }
